...
class CNN_XRayChest(object):
	...
	def create_model1(self):
		...
		self.model.add(Dense(64, kernel_regularizer=regularizers.l2(reg)))
		self.model.add(BatchNormalization())
		# Cria camada com função de ativação ReLU.
		self.model.add(Activation('relu'))
		# Cria camada de dropout para desligar neurônios.
	...
	def save_model(self):
		...
		open('xray-chest_architecture-10000.json', 'w').write(model_json)
		# salva os pesos aprendidos pela rede nos dados de treinamento.
		self.model.save_weights('xray-chest_weights-10000.h5', overwrite=True)
