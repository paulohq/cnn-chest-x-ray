...
class CNN_XRayChest(object):
    def __init__(self):
        ...
        #self.PATH = '/content'
        #self.data_path = self.PATH + '/data-bbox-450'
        ...
        # otimizador
        self.otimizador_sgd = SGD(lr=0.001, decay=1e-6, momentum=0.9, nesterov=True)
        self.otimizador_adam = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0005, amsgrad=True)
        self.otimizador_rmsprop = RMSprop(lr=0.001, decay=1e-6)
        ...
        self.qtde_imagens = 450  # 638

    def open_CSV(self):
        # Abre arquivo csv com o nome das imagens e suas respectivas classes.
        with open(self.PATH + '/BBox_List_2017.csv', newline='') as csvfile:
    ...
    def augmenting_dataset(self):
        ...
        img_flip_ud1 = np.flipud(img_flip_ud)
        img_flip_ud2 = np.flipud(img_flip_ud)
        img_flip_ud = np.flipud(img_flip_ud)
        # pega 200 imagens do dataset de treinamento.
        ...
        img_flip_lr1 = np.fliplr(img_flip_lr)
        img_flip_lr2 = np.fliplr(img_flip_lr)
        img_flip_lr3 = np.fliplr(img_flip_lr)
        # pega 200 imagens do dataset de treinamento
        ...
        img_flip1 = np.flip(img_flip, axis=0)
        img_flip2 = np.flip(img_flip, axis=0)
        img_flip3 = np.flip(img_flip, axis=0)
    ...
    def cnn(self):
        ...
        # self.model.compile(loss='mean_squared_error', optimizer=self.otimizador_sgd, metrics=["accuracy"])
        self.model.compile(loss='categorical_crossentropy', optimizer=self.otimizador_adam, metrics=["accuracy"])
        ...
        self.history = self.model.fit(self.X_train, self.y_train, batch_size=self.batch_size, epochs=self.num_epoch,
                                      verbose=self.verbose, validation_split=self.validation_split, shuffle=True, initial_epoch=40)

        # testa o modelo com os dados separados para teste com o tamanho de batch definido.
        score = self.model.evaluate(self.X_test, self.y_test, batch_size=self.batch_size_test, verbose=self.verbose)
    ...
    def predict(self):
        ...
        model_architecture = 'xray-chest_architecture.json'
        # carrega os pesos aprendidos pela rede no treinamento
        model_weights = 'xray-chest_weights.h5'
        # Lê o modelo
        ...
        print(self.lista_imagens_nome[0:4])
